...
export const getFileType = (filename, contentType) => {
  ...
  // Image types
  if (contentType?.startsWith('image/') || ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp'].includes(extension)) {
    return 'image';
  }
  
  // Video types
  if (contentType?.startsWith('video/') || ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mkv'].includes(extension)) {
    return 'video';
  }
  
  // Audio types
  if (contentType?.startsWith('audio/') || ['mp3', 'wav', 'flac', 'aac', 'ogg', 'm4a'].includes(extension)) {
    return 'audio';
  }
  ...
  if (contentType?.includes('text/') || ['txt', 'md', 'json', 'xml', 'csv', 'log'].includes(extension)) {
    return 'document';
  }
